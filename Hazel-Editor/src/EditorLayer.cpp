...
namespace Quantum {
	...
	void EditorLayer::OnAttach()
	{
		...
		Quantum::FramebufferSpecification fbSpec;
		fbSpec.Width = 1280;
		fbSpec.Height = 720;
		m_Framebuffer = Quantum::Framebuffer::Create(fbSpec);
	}
	...
	void EditorLayer::OnUpdate(Quantum::Timestep ts)
	{
		QT_PROFILE_FUNCTION();
...

		{
			...
			Quantum::Renderer2D::DrawQuad({ 0.5f, -0.5f }, { 0.5f, 0.75f }, m_SquareColor);
			Quantum::Renderer2D::DrawQuad({ 0.0f, 0.0f, -0.1f }, { 20.0f, 20.0f }, m_CheckerboardTexture, 10.0f);
			Quantum::Renderer2D::DrawRotatedQuad({ -2.0f, 0.0f, 0.0f }, { 1.0f, 1.0f }, rotation, m_CheckerboardTexture, 20.0f);
		...
		if (dockingEnabled)
		{
			...
			if (ImGui::BeginMenuBar())
			{
				if (ImGui::BeginMenu("File"))
				{
					...
					if (ImGui::MenuItem("Exit")) Quantum::Application::Get().Close();
					ImGui::EndMenu();
				}
	...
	void EditorLayer::OnEvent(Quantum::Event& e)
	{
